...
def get_traffic_data():
    ...
    pipeline.extend([
        {"$group": {
            ...
            "ports": {"$addToSet": "$dst_port"} # Aggregate destination ports
        }},
        {"$project": {
            "_id": 0, "source": "$_id.source", "target": "$_id.target", "value": "$value",
            "ports": "$ports", # Pass the ports array
            "protocol": {"$reduce": {
    ...
    try:
        ...
        processed_flows, consolidated_links = set(), []
        for flow in flows:
            source, target = flow['source'], flow['target']
            if (source, target) in processed_flows: continue
            ...
            if reverse_flow:
                ...
                combined_ports = sorted(list(set(filter(None, flow.get('ports', []))) | set(filter(None, reverse_flow.get('ports', [])))))

                consolidated_links.append({
                    "source": node_name_map[source], "target": node_name_map[target],
                    "value": combined_value, "protocol": combined_protocol, "ports": combined_ports
                })
                ...
                consolidated_links.append({
                    "source": node_name_map[source], "target": node_name_map[target],
                    "value": flow['value'], "protocol": flow['protocol'].strip(', '), "ports": ports
                })
                processed_flows.add((source, target))
